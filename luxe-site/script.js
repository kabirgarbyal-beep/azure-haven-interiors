...
(() => {
  ...
  if (!reduced) {
    ...
    for (let i = 0; i < N; i++) {
      ...
      e.style.opacity = (Math.random() * 0.5 + 0.25).toFixed(2);
      pc.appendChild(e);
    }
  }
  ...
  $$(".reveal-words").forEach((el) => {
    ...
    el.innerHTML = words
      .map((w, i) => `<span class="word" style="--i:${i}"><i>${w}</i></span>`)
      .join(" ");
  });
  ...
  const restart = () => {
    ...
    qTimer = setInterval(() => go(qi + 1), 6000);
  };
  $$(".qbtn").forEach((b) =>
    b.addEventListener("click", () => go(qi + (b.dataset.q === "next" ? 1 : -1))),
  );
  restart();
  ...
  if (!reduced) {
    ...
    document.addEventListener(
      ...
      () => {
        ...
        if (glowA) glowA.style.transform = `translateY(${y * 0.25}px)`;
        if (glowB) glowB.style.transform = `translateY(${y * 0.15}px)`;
      },
      { passive: true },
    );
